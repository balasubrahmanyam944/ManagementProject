...
class NangoService {
  ...
  private async getConnectionDirect(
    ...
    // Self-hosted Nango uses /connection/ (singular), not /connections/ (plural)
    const path = `/connection/${connectionId}?provider_config_key=${provider}`;
    return this.httpRequest<NangoConnection>('GET', path);
  }
  ...
  async isConnected(
    ...
    try {
      ...
      const connection = await this.getConnectionDirect(provider, connectionId);
  ...
  async deleteConnection(
    ...
    try {
      ...
      // Use direct HTTP call instead of SDK
      await this.deleteConnectionDirect(provider, connectionId);
