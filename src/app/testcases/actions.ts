...
export async function sendTestCaseToTrelloAction(
  ...
  try {
    ...
    if (integration.metadata?.nangoManaged) {
      ...
      const tenantId = integration.metadata.tenantId || 'default';
      trelloToken = await nangoService.getAccessToken('trello', tenantId, userId);
      console.log(`🔑 Trello: Retrieved OAuth1 token from Nango (length: ${trelloToken?.length || 0})`);
    } else {
      console.log(`🔑 Trello: Using DB-stored token (length: ${trelloToken?.length || 0})`);
    }
...
_Generated by UPMY Test Case Generator_
    ...
    const trelloUrl = `https://api.trello.com/1/cards?key=${TRELLO_API_KEY}&token=${trelloToken}`;
    console.log(`🚀 Trello: Creating card on board ${boardId}, list ${listId}`);
    console.log(`🔗 Trello: API URL (token redacted): https://api.trello.com/1/cards?key=${TRELLO_API_KEY}&token=***`);
    
    const createCardResponse = await fetch(trelloUrl, {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
