...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`📄 Server-side PDF extraction: Starting for ${file.name}`);
    ...
    try {
      ...
      console.log(`📄 Server-side PDF extraction: Extracted ${text.length} characters using ${method}`);
      console.log(`📄 Server-side PDF extraction: Preview: ${text.substring(0, 300)}...`);
      ...
      console.error('❌ Server-side PDF extraction error:', pdfError);
      return NextResponse.json({
