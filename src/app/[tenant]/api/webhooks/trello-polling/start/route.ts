/**
 * Trello Polling Start API (Tenant-specific)
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 TRELLO POLLING START API (TENANT): Request received for user', userId)
    
    // Start polling for user's Trello boards
    await startPollingForUser(userId)
    
    console.log('✅ TRELLO POLLING START API (TENANT): Successfully started polling for user', userId)
    ...
    console.error('❌ TRELLO POLLING START API (TENANT): Error:', error)
    return NextResponse.json(
