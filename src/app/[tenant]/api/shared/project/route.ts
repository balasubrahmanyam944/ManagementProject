...
/**
 * Tenant-specific endpoint for shared projects
 * This allows shared projects to be created and viewed from any tenant
 */
...
function convertTestRailTestCasesToIssues(testCases: any[]): any[] {
  return testCases.map((tc) => {
    const status = tc.custom_status_id === 1 ? 'Passed' : tc.custom_status_id === 5 ? 'Failed' : 'Untested';
    return {
      id: tc.id.toString(),
      key: `TC-${tc.id}`,
      summary: tc.title,
      status,
      type: tc.type_id === 1 ? 'Smoke' : tc.type_id === 2 ? 'Regression' : 'Functional',
      priority: tc.priority_id === 1 ? 'High' : tc.priority_id === 2 ? 'Medium' : 'Low',
      assignee: tc.custom_assignee || 'Unassigned',
      created: tc.created_on ? new Date(tc.created_on * 1000).toISOString() : undefined,
      updated: tc.updated_on ? new Date(tc.updated_on * 1000).toISOString() : undefined,
    };
  });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const shareUrl = `${publicUrl}${basePath}/shared/project/${shareId}`;
