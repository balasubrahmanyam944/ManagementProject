...
function IntegrationsContent() {
      ...
      if (integrationLower === 'jira' || integrationLower === 'trello' || integrationLower === 'slack') {
        ...
        try {
          ...
          if (syncResponse.ok) {
            ...
            // Check if webhooks were registered
            if (syncData.webhookRegistered === false && syncData.webhookError) {
              console.warn(`⚠️ INTEGRATIONS PAGE: Webhook registration failed: ${syncData.webhookError}`);
              toast({
                title: `${integrationName} Connected`,
                description: `Connected successfully, but webhooks couldn't be registered. Real-time updates may be delayed. You can manually register webhooks from the integrations page.`,
                variant: "default",
              });
            } else if (syncData.webhookRegistered === true) {
              console.log(`✅ INTEGRATIONS PAGE: Webhooks registered successfully for ${integrationName}`);
            }
  ...
  const handleRegisterWebhooks = async (integrationName: string) => {
    setLoadingStates(prev => ({ ...prev, [`${integrationName}_webhooks`]: true }));
    ...
    try {
      if (!user?.id) {
        ...
        setLoadingStates(prev => ({ ...prev, [`${integrationName}_webhooks`]: false }));
        return;
      }
      ...
      const response = await fetch(`${basePath}/api/webhooks/manage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ integration: integrationLower }),
      });
      ...
      if (result.success) {
        toast({
          title: `${integrationName} Webhooks Registered`,
          description: `Webhooks have been successfully registered. You'll now receive real-time notifications for ${integrationName} changes.`,
        });
      } else {
        // Extract error messages from nested results structure
        const integrationResult = result.results?.[integrationLower];
        const errors = integrationResult?.errors || [];
        const singleError = integrationResult?.error || result.error;
        const errorMsg = errors.length > 0 
          ? errors.join('; ') 
          : (singleError || 'Unknown error');
        toast({
          title: "Webhook Registration Failed",
          description: `Failed to register webhooks: ${errorMsg}. Polling will be used as a fallback.`,
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error(`Error registering webhooks for ${integrationName}:`, error);
      toast({
        title: "Webhook Registration Error",
        description: `Failed to register webhooks for ${integrationName}. Polling will be used as a fallback.`,
        variant: "destructive",
      });
    } finally {
      setLoadingStates(prev => ({ ...prev, [`${integrationName}_webhooks`]: false }));
                ...
                <CardFooter>
                  {loadingStates[integration.name] || (isPending && loadingStates[integration.name] !== false) ? (
                    ...
                    <div className="flex w-full items-center justify-between gap-2">
                      ...
                      {(integration.name === "Jira" || integration.name === "Trello") && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleRegisterWebhooks(integration.name)}
                          disabled={loadingStates[`${integration.name}_webhooks`] || isPending}
                          title="Register webhooks for real-time notifications"
                        >
                          {loadingStates[`${integration.name}_webhooks`] ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                          ) : (
