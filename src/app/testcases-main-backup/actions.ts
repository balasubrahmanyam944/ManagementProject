...
async function getTrelloBoards(): Promise<Board[]> {
  ...
  const trelloToken = integration?.accessToken;
  const TRELLO_API_KEY = process.env.TRELLO_API_KEY;
  if (!trelloToken || integration.status !== 'CONNECTED') throw new Error('Trello not connected');
  if (!TRELLO_API_KEY) throw new Error('Trello API key not configured');
  // Get user's boards
  ...
  if (!boardsResponse.ok) {
    ...
    throw new Error(errorData.message || 'Failed to fetch Trello boards');
  }
...
export async function sendTestCaseToJiraAction(
  ...
  try {
    ...
    // Get the site URL from integration metadata
    const jiraSiteUrl = integration.serverUrl;
    if (!jiraSiteUrl) {
      return {
        success: false,
...
export async function sendTestCaseToTrelloAction(
  ...
  try {
    ...
    const trelloToken = integration?.accessToken;
    const TRELLO_API_KEY = process.env.TRELLO_API_KEY;
    if (!trelloToken || integration.status !== 'CONNECTED') {
      return {
        success: false,
...
_Generated by UPMY Test Case Generator_
    ...
    if (!createCardResponse.ok) {
      ...
      throw new Error(errorData.message || 'Failed to create Trello card');
    }
    const result = await createCardResponse.json();
